...
//		var result = await db.rawQuery('SELECT * FROM $todoTable order by $colTitle ASC');
  ...
  Future<int> insertTodo(Challenge todo) async {
    Database db = await this.database;
  ...
  Future<int> updateTodo(Challenge todo) async {
  ...
  Future<List<Challenge>> getTodoList() async {
    ...
    List<Challenge> todoList = List<Challenge>();
    // For loop to create a 'todo List' from a 'Map List'
    for (int i = 0; i < count; i++) {
      todoList.add(Challenge.fromMapObject(todoMapList[i]));
    }
